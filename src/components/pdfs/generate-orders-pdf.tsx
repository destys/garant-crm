...
export const generateOrdersReportPdf = (
  ...
  // заголовок
  let titleText = activeTitle?.trim() || "Отчёт по заявкам";
  if (period?.from && period?.to) {
    titleText += ` (${format(period.from, "dd.MM.yyyy", {
      locale: ru,
  ...
  doc.setFontSize(14);
  doc.setFont(FONT, "normal");
  doc.text(titleText, pageW / 2, margin, { align: "center" });
  doc.setFontSize(9); // вернуть базовый

  // заголовок
  if (period?.from && period?.to) {
    doc.setFontSize(11);
    const title = `${format(period.from, "dd.MM.yyyy", {
      locale: ru,
    })} — ${format(period.to, "dd.MM.yyyy", { locale: ru })}`;
  ...
  const cards = orders.map((o) => {
    const number =
      o.title?.trim() || (o.id ? `#${o.id}` : o.documentId?.slice(0, 8) || "—");
  ...
  cards.forEach((card, idx) => {
    ...
    const labelMaxW = Math.max(
      ...card.lines.map((l) => doc.getTextWidth(l.label))
    );
