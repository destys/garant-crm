...
export const OrderChat = ({ data }: { data: OrderProps }) => {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const newMessage = {
      ...
      user: { id: user.id, name: user.name || user.username || "Пользователь" },
      datetime: new Date().toISOString(),
    };

    // Убираем лишние поля у старых сообщений, сохраняя user: { id, name }
    const sanitizedChat = (currentChat || []).map((msg) => ({
      message: msg.message,
      user: {
        id: msg.user?.id,
        name: msg.user?.name || "Пользователь",
      },
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between gap-5 mb-5 text-xs opacity-70">
                      ...
                      <div>{m.user?.name || "Пользователь"}</div>
                    </div>
